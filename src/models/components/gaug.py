...
class VGAE(nn.Module):
    ...
    def __init__(self, dim_feats, conf):
        ...
        self.encoder = GCNEncoder(
            ...
            conf.gsl["n_layers"],
            conf.gsl["dropout"],
            conf.gsl["input_dropout"],
            conf.gsl["norm"],
            conf.gsl["n_linear"],
            conf.gsl["spmm_type"],
            conf.gsl["act"],
            conf.gsl["input_layer"],
            conf.gsl["output_layer"],
            bias=False,
...
class GAug(nn.Module):
    def __init__(self, dim_feats, n_classes, conf):
        ...
        if conf.model["type"] == "gcn":
            self.nc_net = GCNEncoder(
                dim_feats,
                conf.model["n_hidden"],
                n_classes,
                conf.model["n_layers"],
                conf.model["dropout"],
                conf.model["input_dropout"],
                conf.model["norm"],
                conf.model["n_linear"],
                conf.model["spmm_type"],
                conf.model["act"],
                conf.model["input_layer"],
                conf.model["output_layer"],
                weight_initializer="glorot",
                bias_initializer="zeros",
            )
        elif conf.model["type"] == "appnp":
            self.nc_net = APPNPEncoder(
                dim_feats,
                conf.model["n_hidden"],
                n_classes,
            ...
            self.nc_net = GINEncoder(
                dim_feats,
                conf.model["n_hidden"],
                n_classes,
                conf.model["n_layers"],
                conf.model["mlp_layers"],
            )
