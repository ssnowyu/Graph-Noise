...
from dgl.data import DGLDataset, CiteseerGraphDataset
import torch
...
class CiteSeerDataset(DGLDataset):
    def __init__(
        ...
        super().__init__(
            name="CiteSeer", raw_dir=raw_dir, save_dir=save_dir, force_reload=force_reload
        )

    def download(self):
        CiteseerGraphDataset(raw_dir=self.raw_dir, reverse_edge=False)

    def process(self):
        g = CiteseerGraphDataset(
...
if __name__ == "__main__":
    dataset = CiteSeerDataset(raw_dir="data/cora/raw", feat_noise_rate=0.6, feat_sigma=0.2)
    g = dataset[0]
    feat = g.ndata["feat"]
    # print(feat.sum(1))
